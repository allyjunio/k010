...
//Sidebar Controller
app.controller('SidebarCtrl', function ($scope, $state, $rootScope, $timeout, SidebarList, SAVE_SETTING, $localStorage, StorageGet, paginasStates, Restangular) {

	while($rootScope.loading){
		//wait
	}
	$scope.ace = $scope.$parent.ace;
	...
	Restangular.one('user').get().then(function(user){
		var paginas = [];
		for(var i = 0; i < user.roles[0].perfilAcesso.length; i++){
			paginas.push(paginasStates.getPagina(user.roles[0].perfilAcesso[i]));
		}
		$scope.sidebar = SidebarList.getList(paginas);
